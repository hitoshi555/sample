...
<template>
  ...
  <div class="mx-auto grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 lg:mx-0 lg:max-w-none lg:items-start lg:gap-y-10">
    <div class="lg:col-span-2 lg:col-start-1 lg:row-start-1 lg:mx-auto lg:grid lg:w-full lg:max-w-7xl lg:gap-x-8 lg:px-8">
      <div class="lg:pr-4">
        <div class="lg">
    ...
    <ul role="list" class="grid gap-x-8 gap-y-12 sm:grid-cols-2 sm:gap-y-16 xl:col-span-2">
      <li v-for="teacher in teachers" :key="teacher.id">
        ...
        <div class="flex items-center gap-x-6">
         <div>
            <h3 class="text-base font-semibold leading-7 tracking-tight text-gray-900">{{ teacher.name }}</h3>
            <p class="text-sm font-semibold leading-6 text-indigo-600">{{teacher.position}}</p>
          </div>
        </div>
      </li>
    </ul>
